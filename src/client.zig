...
pub fn attach(alloc: std.mem.Allocator, socket_path: []const u8) !Outcome {
    ...
    // connection or miss the initial size. A plain attach is raw
    // passthrough and sends no `.ui` marker, so the daemon does not
    // replay scrollback (which would dump the buffer onto the user's
    // real terminal).
