...
fn cmdNew(alloc: std.mem.Allocator, args: []const [:0]const u8) !void {
    ...
    return createSession(alloc, dir, name, detached, @ptrCast(cmd_argv), null, rows, cols, cwd);
}
...
fn createSession(
    ...
    // A restore cwd (restore_cwd) is already known-good and is used as-is.
...
/// Re-create sessions from their last snapshot in their saved working
/// directory. A surviving daemon sweeps the snapshot of any session that
/// ended while it ran, so what remains to restore is whatever outlived a
/// full shutdown (`kill --all`, a crash, or a reboot). With a name,
/// restores just that session; with none, every session that has a saved
/// snapshot but no live socket.
...
fn cmdUi(alloc: std.mem.Allocator, args: []const [:0]const u8) !void {
    ...
    const result = ui.run(alloc, dir, max_scrollback);
...
fn inspectHuman(alloc: std.mem.Allocator, text: []const u8, sock: []const u8) !void {
    ...
    try out.print(alloc, "Scrollback:  {s} bytes\n", .{inspectField(text, "max_scrollback")});
...
fn inspectJson(alloc: std.mem.Allocator, text: []const u8, sock: []const u8) !void {
    ...
    const idle_ms = std.fmt.parseInt(i64, inspectField(text, "idle_ms"), 10) catch 0;
    const out_idle_ms = std.fmt.parseInt(i64, inspectField(text, "out_idle_ms"), 10) catch 0;
    const max_scrollback = std.fmt.parseInt(i64, inspectField(text, "max_scrollback"), 10) catch 0;
    ...
    const tail = try std.fmt.allocPrint(alloc, ",\"max_scrollback\":{d},\"title\":", .{max_scrollback});
    defer alloc.free(tail);
    try out.appendSlice(alloc, tail);
